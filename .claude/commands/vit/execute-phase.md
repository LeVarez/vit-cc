...
0.5. **Verify worktree and branch**
   ...
   **Resolve per-plan branches:**
   ...
   ```bash
   PLAN_BRANCHES=$(grep "| ${MILESTONE}/${PHASE_NUM} " "$WORK_DIR/.planning/STATE.md" 2>/dev/null \
     | grep -o 'Plan Branches' || echo "")
   HAS_PLAN_BRANCHES=false
   if [ -n "$PLAN_BRANCHES" ]; then
     # Extract plan branch names from the Plan Branches column
     PLAN_BRANCH_LIST=$(grep "| ${MILESTONE}/${PHASE_NUM} " "$WORK_DIR/.planning/STATE.md" 2>/dev/null \
       | sed 's/.*Plan Branches[^|]*|//' | head -1 | tr ',' '\n' | tr -d ' ' | grep -v '^$')
     if [ -n "$PLAN_BRANCH_LIST" ]; then
...
4. **Execute waves**
   ...
   **Per-plan branch mode** (`HAS_PLAN_BRANCHES=true`):
   - For each plan in the wave, resolve its plan branch:
     ...
     PLAN_BRANCH="feature/v${MILESTONE}-${PHASE_NUM}-${PLAN_SLUG}"
     ```
