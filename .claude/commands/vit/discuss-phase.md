...
<execution_context>
@./.claude/vit/workflows/discuss-phase.md
@./.claude/vit/templates/context.md
</execution_context>
...
6. **Write CONTEXT.md** — Sections match areas discussed. Write to `"$WORK_DIR/.planning/phases/${MILESTONE}/{NN}-{name}/{NN}-CONTEXT.md"` (for milestone-scoped phases) or `"$WORK_DIR/.planning/phases/{NN}-{name}/{NN}-CONTEXT.md"` (for historical phases without a milestone dir). Use the same path structure as the existing phase directory. Commit:
   ```bash
   cd "$WORK_DIR" && git add .planning/phases/${MILESTONE}/{NN}-{name}/{NN}-CONTEXT.md
   cd "$WORK_DIR" && git commit -m "docs({phase}): gather phase context"
   ```
