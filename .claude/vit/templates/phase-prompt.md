...
> **Note:** Planning methodology is in `agents/vit-planner.md`.
...
<!-- For checkpoint task examples and patterns, see @./.claude/vit/references/checkpoints.md -->
...
**Wave is pre-computed:** Wave numbers are assigned during `/vit:plan-phase`. Execute-phase reads `wave` directly from frontmatter and groups plans by wave number. No runtime dependency analysis needed.
...
See `./.claude/vit/templates/user-setup.md` for full schema and examples
...
See `./.claude/vit/workflows/verify-phase.md` for verification logic.
