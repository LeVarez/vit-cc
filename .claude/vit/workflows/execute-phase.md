...
<step name="create_draft_pr">
Create a draft PR for this phase on GitHub. Runs after worktree/branch validation, before plan discovery. Non-blocking — gracefully skips on any failure.
...
- Write `pr:skipped` to the PR column in the phase's STATE.md row
- Continue to discover_plans

**2. Idempotency check (always query live, never trust STATE.md alone):**
```bash
...
- Ensure STATE.md PR column shows `pr#$EXISTING_PR` (update if `—`)
- Continue to discover_plans — do NOT create a duplicate

**3. Assemble PR body from planning artifacts:**
...
# Phase blockers from STATE.md (omit ## Notes section if empty)
...
- `## Plans` — checkbox list of all plans (`- [ ] NN-NN: objective`)
- `## Success Criteria` — observable behaviors from ROADMAP.md
- `## Notes` — blockers from STATE.md (omit entire section if empty)
- `## Links` — feature issue link and `Closes #N`
...
# Update the PR column from — to pr#N
cd "$WORK_DIR" && sed -i '' "s/| ${MILESTONE}\/${PHASE_NUM} \([^|]*\)| \([^|]*\)| \([^|]*\)| —[[:space:]]*/| ${MILESTONE}\/${PHASE_NUM} \1| \2| \3| pr#${PR_NUM} /" .planning/STATE.md 2>/dev/null || true
```

On failure (empty `NEW_PR`):
- Log: `[PR skipped — creation failed]`
- Write `pr:skipped` to PR column in STATE.md
- Continue to discover_plans (non-blocking)

**PR column values:**
- `—` — no PR yet (initial value from new-milestone)
- `pr#N` — draft PR created
- `pr#N(ready)` — promoted to ready-for-review (written by verify-work)
- `pr:skipped` — gh unavailable or creation failed
</step>
...
**No dependency analysis needed.** Wave numbers are pre-computed during `/vit:plan-phase`.
...
1. **Spawn agent for checkpoint plan:**
   ```
   Task(prompt="{subagent-task-prompt}", subagent_type="vit-executor", model="{executor_model}")
   ```
...
| `gaps_found` | Present gap summary, offer `/vit:plan-phase {phase} --gaps` |
...
- `/vit:verify-work {X}` — manual testing before planning
```

User runs `/vit:plan-phase {X} --gaps` which:
1. Reads VERIFICATION.md gaps
2. Creates additional plans (04, 05, etc.) with `gap_closure: true` to close gaps
3. User then runs `/vit:execute-phase {X} --gaps-only`
...
1. Run `/vit:execute-phase {phase}` again
