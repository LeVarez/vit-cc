...
2. Use Task tool with subagent_type="vit-executor" and model="{executor_model}":
...
2. For each segment in order:
   ...
   B. If routing = Subagent:
      ```
      Spawn Task tool with subagent_type="vit-executor" and model="{executor_model}":
...
See `./.claude/vit/references/tdd.md` for TDD plan structure.
</tdd_plan_execution>
...
See ./.claude/vit/references/checkpoints.md for complete checkpoint guidance.
</step>
...
Create `.planning/phases/XX-name/{phase}-USER-SETUP.md` using template from `./.claude/vit/templates/user-setup.md`.
...
**Also available:**
- `/vit:verify-work {phase}-{plan}` — manual acceptance testing before continuing
...
- `/vit:discuss-phase {Z+1}` — gather context first
...
**Also available:**
- `/vit:verify-work` — manual acceptance testing before completing milestone
- `/vit:add-phase <description>` — add another phase before completing
