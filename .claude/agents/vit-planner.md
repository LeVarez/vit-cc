...
- `/vit:plan-phase` orchestrator in revision mode (updating plans based on checker feedback)
...
For niche domains (3D, games, audio, shaders, ML), suggest `/vit:research-phase` before plan-phase.
...
# Detect milestone if not passed by orchestrator
if [ -z "$MILESTONE" ]; then
...
if [ -z "$PHASE_DIR" ]; then
  PHASE_DIR=$(ls -d "$WORK_DIR/.planning/phases/$PADDED_PHASE-"* "$WORK_DIR/.planning/phases/$PHASE_ARG-"* 2>/dev/null | head -1)
fi
...
```bash
cat "$WORK_DIR"/.planning/phases/$PHASE-*/*-PLAN.md
```
...
```bash
cd "$WORK_DIR" && git add .planning/phases/$PHASE-*/$PHASE-*-PLAN.md
cd "$WORK_DIR" && git commit -m "fix($PHASE): revise plans based on checker feedback"
```
...
# Read RESEARCH.md if exists (from /vit:research-phase)
...
- `**Plans:** (created by /vit:plan-phase)` → `**Plans:** {N} plans`
...
```bash
cd "$WORK_DIR" && git add .planning/phases/$PHASE-*/$PHASE-*-PLAN.md .planning/ROADMAP.md
cd "$WORK_DIR" && git commit -m "docs($PHASE): create phase plan
...
Execute: `/vit:execute-phase {phase}`
...
Execute: `/vit:execute-phase {phase} --gaps-only`
```
...
- [ ] PLAN file(s) committed to git
- [ ] User knows to run `/vit:execute-phase {X}` next
