...
- Example: `/vit:remove-phase 17` → phase = 17
- Example: `/vit:remove-phase 16.1` → phase = 16.1

If no argument provided:

```
ERROR: Phase number required
Usage: /vit:remove-phase <phase-number>
Example: /vit:remove-phase 17
```
...
To abandon current work, use /vit:pause-work instead.
```
...
```bash
ls .planning/phases/${MILESTONE}/{target}-*/*-SUMMARY.md 2>/dev/null || ls .planning/phases/{target}-*/*-SUMMARY.md 2>/dev/null
```
...
```bash
if [ -d ".planning/phases/${MILESTONE}/{target}-{slug}" ]; then
  rm -rf ".planning/phases/${MILESTONE}/{target}-{slug}"
  echo "Deleted: .planning/phases/${MILESTONE}/{target}-{slug}/"
elif [ -d ".planning/phases/{target}-{slug}" ]; then
...
mv ".planning/phases/${MILESTONE}/18-dashboard" ".planning/phases/${MILESTONE}/17-dashboard" 2>/dev/null || \
...
grep -r "Phase 18" .planning/phases/${MILESTONE}/17-*/ 2>/dev/null || grep -r "Phase 18" .planning/phases/17-*/ 2>/dev/null
grep -r "Phase 19" .planning/phases/${MILESTONE}/18-*/ 2>/dev/null || grep -r "Phase 19" .planning/phases/18-*/ 2>/dev/null
# etc.
```
...
Look up feature issue and branch for the removed phase from STATE.md `## GitHub Issue Mapping` table.

If a mapping row is found for Phase {target}:

```bash
gh issue close $FEATURE_ISSUE \
  --comment "Phase {target} removed from roadmap" 2>/dev/null || true
git push origin --delete "feature/${MILESTONE}-{target}-{slug}" 2>/dev/null || true
```

Remove the row for Phase {target} from the `## GitHub Issue Mapping` table in STATE.md.
</step>
...
Would you like to:
- `/vit:progress` — see updated roadmap status
