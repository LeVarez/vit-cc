...
| Review scoped to phase diff, not full codebase | Reviewing only what changed in this phase avoids noise from existing code debt | LOW | Pass `git diff milestone/vX.Y.Z...feature/vX.Y.Z-N` to reviewer agent, not full codebase scan. |
