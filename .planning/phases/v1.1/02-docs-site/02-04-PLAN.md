...
1. **Branch Naming Convention** — `milestone/v{X.Y}` for milestone branches, `feature/v{X.Y}-{NN}-{slug}` for feature branches, `feature/v{X.Y}-{NN}-{PP}` for plan branches. Show the hierarchy and how branches relate.
