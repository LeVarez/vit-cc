...
|-------|---------|---------|---------|
| Milestone | `milestone/v{X.Y.Z}` | `milestone/v1.1.0` | Base branch for all phase work in a milestone |
| Feature (phase) | `feature/v{X.Y.Z}-{NN}-{slug}` | `feature/v1.1.0-02-docs-site` | One branch per phase; PRs target the milestone branch |
| Plan | `feature/v{X.Y.Z}-{NN}-{PP}` | `feature/v1.1.0-02-04` | Worktree branch for parallel plan execution |
...
```bash
gh issue create \
  --title "feat(v1.1.0): Phase 02 — docs-site" \
...
gh issue develop $FEATURE_ISSUE \
  --base "milestone/v1.1.0" \
  --name "feature/v1.1.0-02-docs-site"
```
...
1. **Type check** — Runs `npm run type-check` (TypeScript). Continues on error so tests still run.
2. **Phase tests** — Runs Vitest against `tests/phases/` if tests exist. Supports both milestone-scoped (`tests/phases/v1.6.0/`) and legacy (`tests/phases/`) layouts.
...
```
## CI — Phase v1.1.0/02 @ `a1b2c3d`

**Branch:** `feature/v1.1.0-02-docs-site`
...
```
## CI Passed — Phase v1.1.0/02 @ `a1b2c3d`
...
```
## CI Failed — Phase v1.1.0/02 @ `a1b2c3d`
...
|-------|---------------|--------|----|----------|------------|
| v1.1.0/01 | #47 | feature/v1.1.0-01-readme-rewrite | pr#53(ready) | — | #49, #50, #51, #52 |
| v1.1.0/02 | #48 | feature/v1.1.0-02-docs-site | pr#66 | — | #61, #62, #63, #64, #65 |
```
